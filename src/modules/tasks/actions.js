...
export function createTask(title, type) {
  return (dispatch, getState) => {
    const { auth, firebase } = getState();
console.log('___', title, type);
    firebase.child(`tasks/${auth.id}`)
      .push({completed: false, title, type}, error => {
        if (error) {
...
export function undeleteTask() {
  return (dispatch, getState) => {
    ...
    firebase.child(`tasks/${auth.id}/${task.key}`)
      .set({completed: task.completed, title: task.title, type: task.type}, error => {
        if (error) {
